...
\date{8.\ December 2017}  % if the \date{} command is left out, the current date will be used
...
In order to use cookies, Web browser support must be enabled\marginnote{Cookies must be enabled by the Web browser}.
...
\end{enumerate}
If\marginnote{Usually, cookie data are stored in a database file.} the cookie was accepted, the cookie information is either held in memory or stored on the client's hard drive depending on the Web browser's configuration. % On the client side
...
Some Web browsers such as Google's Chrome browser stores cookie data in a SQLite database\sidenote{For more information see \url{https://stackoverflow.com/questions/31021764/where-does-chrome-store-cookies}}. 
...
\begin{itemize}
	\item \emph{Reading cookie data} \\ The\marginnote{Access restrictions} probably most relevant restriction is that a cookie can only be read by the server that has set the cookie. 
...
\item \emph{Cookie size} \\ A\marginnote{File size} cookie must not exceed the maximum size of 4 kilobyte, i.e., 4096 characters. The size restriction includes both \emph{name} and \emph{value}. If name and value are grater than 4 kilobyte, all characters that exceed this limit will be cut off. 

\item \emph{Amount of Cookies accepted by Web browser} \\ A\marginnote{Number of cookies} Web server is instructed to accept a maximum of 300 cookies at all and a maximum of 20 cookies per domain. In case one of these numbers is exceeded, the oldest cookie (i.e., 301 or 21) will be deleted.    
\end{itemize}
...
Particularly\marginnote{Don't set a lot of single cookies} regarding the second point, it is not a good practice to send a lot of single cookies, e.g., one with each image etc. Another point is that cookies should be used with caution and with thriftiness, i.e., that information should be bundled in one cookie rather than sending a large number of single cookies that would soon exceed the 300 number limit. 
Another\marginnote{Set a realistic expiration date} point concerns the expiration date which should also be set rationally. It is rather stupid to set an expiration date to the far future, e.g., year 2041; inmost cases, one year is more than sufficient.
...
\section{Sessions\marginnote{As we will see, sessions and cookies work perfectly togehter}}% (fold)
\label{sec:sessions}
In the last section, we learned that HTTP is a stateless protocol and that cookies are one way to deal with that issue.
However\marginnote{Cookies might be misused to harm users' privacy by creating user profiles.}, the main problem associated with cookies from an end user perspective is that they can be used to harm users' privacy since they allow 3rd parties to create user profiles---at least to a certain degree. Another\marginnote{Cookies can be disabled} problem is that they can be disabled by the user's Web browser.

\emph{Sessions} are another possibility to deal with the stateless character of HTTP. The\marginnote{Session denotes the act of visiting a website by a user.} term <<session>> usually denotes the act of visiting a Web page by a user. %\marginnote{Accessing a Web site for 10 minutes denotes a 10 minute session; when a user returns after 1 hour to the Web site, usually a new session will be initiated.}.  
...
Using\marginnote{Cookies and sessions work perfectly togehter and are the safest method of storing user data} cookies for storing the Session ID seems counter intuitive since cookies can be disabled in the user's Web browser. However, cookies are the one and only useful method for storing session data since a session management without cookies yields some serious drawbacks, e.g., \emph{session hijacking}.
...
However, PHP can be configured to automatically initiate the session management on \emph{every} page\sidenote{In terms of performance, it is not useful to have session mangement on every page.} using the following option flag in the \texttt{php.ini}:
\begin{Verbatim}
