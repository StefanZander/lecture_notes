...
\begin{itemize}
	...
	% Sinnvoll ist der Einsatz eines Path-Cookie immer dann, wenn nur in einem bestimmten Teilbereich einer Website Cookieinformationen verarbeitet werden müssen. Besteht eine Website beispielsweise sowohl aus einem Cookie-freien statischen, als auch einem dynamischen Seitenbereich, der Cookies verwendet, so lässt sich der Datenaustausch durch geschickte Angabe eines Cookie Path auf den dynamischen Teil der Internetseite beschränken. Angenommen unter dem Verzeichnisbereich /forum/ unter der Domain cookies-path.de ist ein Forum installiert, welches angemeldete Benutzer über einen Cookie identifiziert, während der Rest der Internetseite lediglich statischen Content enthält. Dann sollten Cookies auf den Path /forum/ beschränkt sein, wenn diese lediglich zum Betrieb des Forums benötigt werden.
...
When\marginnote{Session data are stored server-side} PHP stores data in a session object, they need to be serialized beforehand and are then stored on the Web servers file system, depending on its configuration. 
Each \marginnote{The Session ID serves as identifier of the session and the data stored in the session objects} session will be identified through a 32-character long hexadecimal value, the so-called \textbf{Session ID}.
...
The\marginnote{Sensible data should never be stored in cookies!} data are stored on the server-side and can be accessed through a set of PHP convenience methods. 
...
\end{Verbatim}  
As\marginnote{The PHP Interpreter needs to add the Session ID to every URL embedded in a Web page.} a consequence, every link embedded in Web page needs to be automatically extended by the PHP Interpreter using the Session ID so that it is not lost. % This could result in a bottleneck for large pages since those need to be parsed and manipulated prior to their delivery. 
...
\marginnote{Working with sessions mainly involves reading and writing data from/to the session object} When working with sessions in PHP, a large share of programming work is related to writing data to and reading data from the super global session array \texttt{\$\_SESSION}.
...
\subsection{Destroying the Session} % (fold)
\label{sub:destroying_the_session}

% subsection destroying_the_session (end)
...
Sometimes,\marginnote{Delete the session cookie} it might be necessary to also delete the session cookie, if it has been set before. 
