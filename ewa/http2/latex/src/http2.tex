...
Um\marginnote{\textbf{Workaround 1}: Aufbau mehrerer paralleler Verbindungen zum Server} diese Limitierung zu umgehen öffnen Browser üblicherweise sechs parallele Verbindungen, so dass \textbf{maximal sechs Ressourcen gleichzeitig übertragen} werden können\marginnote{HTTP/1.1 erlaubt max. 6 parallele Verbindungen zum Server}. Alle übrigen Requests müssen auf eine freie Verbindung warten\marginnote{Head of Line Blocking}; dieser Flaschenhals wird \textbf{Head of Line Blocking} genannt und ist in Abbildung~\ref{fig:head_of_line_blocking} (linke Hälfte) dargestellt.
