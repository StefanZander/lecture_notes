...
WebSockets create a bi-directional communication channel between a server and a client. This channel can be used by clients and servers likewise to send textual and/or binary data\sidenote{Some websites try to circumvent ad blockers by sending advertisements through WebSocket connections.}. 
...
% Das HTTP-Protokoll\note{und hier kommt noch viel viel mehr text der dann so oder so dargestellt wid sdssdsdds dgdfg dfgdfg dfggdfgd} erlaubt per Definition nur \emph{Request-Response} Kommunikation. Nach jedem Request-Response wird die Verbindung wieder geschlossen. Auch gibt es keine \emph{PUSH}-Benachrichtigung. D.h., eine reine \texttt{Umsetzung} des \texttt{MVC-Paradigmas}\footnote{Model-View-Controller: Ein Architekturprinzip zur besseren Wartbarkeit von Code welches eine Trennung nach der Verantwortlichkeit von Komponenten eines Softwaresystems propagiert.}  ist in Web-Applikationen bisher nicht möglich.

% \fvset{fontsize=\small} %footnotesize
...
%             node.textContent = "Prima! WebSockets werden unterstützt.";
%         } else {
%             node.textContent = "Schade, WebSockets werden nicht von Ihrem Browser unterstützt";
%         }
%     </script>
% </body>
% </html>
...
\begin{figure}%
	\centering
  \includegraphics[width=0.8\textwidth]{./src/figures/long-polling.pdf}
...
\end{Verbatim}
will then be converted into a hash value using the SHA-1\sidenote{\url{https://en.wikipedia.org/wiki/SHA-1}} cryptographic hash function and transformed into a Base64-coded text string. This text string "<\texttt{s3pPLMBiTxaQ9kYGzzhZRbK+xOo=}"> consequently represents the value of the \texttt{Sec-WebSocket-Accept} header.
...
Not every event handler need to be connected with a handler function\sidenote{For instance, a sensor that distributes sensed values (so-called \emph{obserables}) unidirectionally via the WebSocket protocol might only define handler functions for opening or closing a connection but not for receiving information from a server.}; it is absolutely ok to define handler functions exclusively for the events of interest. 
...
From a web server development perspective, Node.js offers a number of benefits, which are listed below:\sidenote{This enumeration is taken from the Modzilla Web Development Portal: \url{https://developer.mozilla.org/en-US/docs/Learn/Server-side/Express_Nodejs/Introduction}}
\begin{itemize}
