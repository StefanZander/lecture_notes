...
Apart from \marginnote{User behaviour on the Web is paradoxial} direct transactions (such as online banking), user behavior in relation to Web content in particular is paradoxical:
\begin{itemize}[--->]
...
\begin{enumerate}
	...
	\item In\marginnote{Users scan using an F-pattern} 2006, eyetracking research found that users frequently scan website prose in an F-pattern, focusing on words at the top or left side of the page, while barely glancing at words that appeared elsewhere.
...
As a hint, think about how you would communicate a specific kind of information in person in the real world.
